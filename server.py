...
def generate_cards():
    global CARDS
    CARDS = [Card(TYPE, NUMBERS[i], i) for TYPE in TYPES for i in range(len(NUMBERS))]
...
def generate_field():
    global FIELD
    FIELD = tuple(
        [TEAMS[j].players[i] for i in range(TEAM_SIZE) for j in range(NUMBER_OF_TEAMS)]
    )
...
def hand_out_cards():
    ...
    for i in range(NUMBER_OF_TEAMS):
        for j in range(TEAM_SIZE):
            TEAMS[i].players[j].set_cards(
                CARDS[
                    cards_per_player * (i * TEAM_SIZE + j) : (i * TEAM_SIZE + j)
                    * cards_per_player
                    + cards_per_player
                ]
            )
...
def fold_first(player: Player):
    ...
    while len(player.hand) > 12:
        player_hand = ", ".join(
            [f"{player.hand[i]}:{i}" for i in range(len(player.hand))]
        )
...
def hand_collector(ground: Ground) -> int:
    if HOKM == NARAS:
        ...
        player_to_collect, min_card = ground.hand[0]
        for card in ground.hand[1:]:
            if card[1].type == ground.type and (
                card[1].ord < min_card.ord or card[1].type == "12"
            ):
                player_to_collect, min_card = card
    else:
        player_to_collect, max_card = ground.hand[0]
...
def client_handler(connection: socket.socket):
    message = "1$_$_$Choose your name:".encode("utf-8")
...
def start_game():
    ...
    while all(team.score < 104 for team in TEAMS):
        ...
        for player in FIELD:
            ...
            player.send_message(
                f"These are your cards: {player_hand}\nWhat is your bet?"
            )
        ...
        while len(f_team.collected_hands) < highest_bet and len(
            b_team.collected_hands
        ) < (14 - highest_bet):
            broadcast_message(
                f"{f_team.name}: {len(f_team.collected_hands)}\n{b_team.name}: {len(b_team.collected_hands)}"
            )
            ground = Ground()
            ...
            while True:
                player_hand = ", ".join(
                    [f"{card}:{i}" for i, card in enumerate(player_to_start.hand)]
                )
                player_to_start.send_message(
                    f"{pre}{player_to_start.name}: {player_hand}\nChoose a card to play:"
                )
            ...
            for i in range(1, 4):
                broadcast_message(
                    ", ".join([f"{p_c[0].name}:{p_c[1]}" for p_c in ground.hand])
                )
                player_to_play = FIELD[(round_starter + i) % len(FIELD)]
                pre = ""
                while True:
                    player_hand = ", ".join(
                        [f"{hand}:{i}" for i, hand in enumerate(player_to_play.hand)]
                    )
                    player_to_play.send_message(
                        f"{pre}{player_to_play.name}: {player_hand}\nChoose a card to play:"
                    )
                    ...
                    if (
                        any(
                            player_card.type == ground.type
                            for player_card in player_to_play.hand
                        )
        ...
        CARDS = [
            card[1] for team in TEAMS for hand in team.collected_hands for card in hand
        ] + [card for player in FIELD for card in player.hand]
        CARDS += ground_cards
        for TEAM in TEAMS:
            TEAM.collected_hands = []
    broadcast_message(
        f"Winner is {filter(lambda team: team.score >= 104, TEAMS)[0].name}!"
    )
