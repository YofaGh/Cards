...
pub fn validate_email(email: &str) -> Result<()> {
    if email.is_empty() {
        return Err(Error::Validator("Email cannot be empty".to_string()));
    }
    if email.len() > 254 {
        return Err(Error::Validator(
    ...
    if !email.contains('@') {
        return Err(Error::Validator("Invalid email format".to_string()));
    }
...
pub fn validate_username(username: &str) -> Result<()> {
    if username.is_empty() {
        return Err(Error::Validator("Username cannot be empty".to_string()));
    }
    if username.len() < 3 {
        return Err(Error::Validator(
