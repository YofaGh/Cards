...
impl UserRepository {
    ...
    pub async fn create_user(
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|err: SqlxError| Error::Database(format!("Failed to create user: {err}")))?;
        Ok(User {
            id: row.id,
            email: row.email,
    ...
    pub async fn delete_user(&self, user_id: UserId) -> Result<()> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|err: SqlxError| Error::Database(format!("Failed to remove user: {err}")))?;
    ...
    pub async fn get_user_by_username(&self, username: String) -> Result<Option<User>> {
        ...
        .await
        .map_err(|err: SqlxError| Error::Database(format!("Failed to get user by id: {err}")))?;
        Ok(row.map(|row| User {
            id: row.id,
            email: row.email,
            password_hash: row.password_hash,
    ...
    pub async fn email_exists(&self, email: &str) -> Result<bool> {
        ...
        .map_err(|err: SqlxError| {
            Error::Database(format!("Failed to check email existence: {err}"))
        })?;
    ...
    pub async fn update_profile(
        ...
        .map_err(|err: SqlxError| {
            if matches!(err, sqlx::Error::RowNotFound) {
                ...
                Error::Database(format!("Failed to update profile: {err}"))
            }
        })?;
        Ok(User {
            id: row.id,
    ...
    pub async fn update_game_stats(
        ...
        .execute(&self.pool)
        .await
        .map_err(|err: SqlxError| Error::Database(format!("Failed to update game stats: {err}")))?;
        if result.rows_affected() == 0 {
            return Err(Error::UserIdNotFound(user_id));
        }
        Ok(())
    ...
    pub async fn lock_user(&self, user_id: UserId) -> Result<()> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|err: SqlxError| Error::Database(format!("Failed to lock user: {err}")))?;
        if result.rows_affected() == 0 {
            return Err(Error::UserIdNotFound(user_id));
        }
        Ok(())
    ...
    pub async fn unlock_user(&self, user_id: UserId) -> Result<()> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|err: SqlxError| Error::Database(format!("Failed to unlock user: {err}")))?;
        if result.rows_affected() == 0 {
            return Err(Error::UserIdNotFound(user_id));
        }
        Ok(())
