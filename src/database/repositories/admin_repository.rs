...
impl AdminRepository {
    ...
    pub async fn create_admin(
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e: SqlxError| Error::Other(format!("Failed to create admin: {}", e)))?;
        Ok(Admin {
            id: row.id,
            email: row.email,
    ...
    pub async fn delete_admin(&self, admin_id: UserId) -> Result<()> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| Error::Other(format!("Failed to remove admin: {}", e)))?;
    ...
    pub async fn get_admin_by_id(&self, admin_id: UserId) -> Result<Option<Admin>> {
        ...
        .await
        .map_err(|e: SqlxError| Error::Other(format!("Failed to get admin by id: {}", e)))?;
